...
function solution(bridge_length, weight, truck_weights) {
    ...
    while(truck_weights.length > 0 || onBridge.length > 0){
        time++;
        if(onTime[0]+bridge_length == time){
            onBridge.shift();
            onTime.shift();
        }
        if(onBridge.reduce((a, c) =>  a + c, 0) + truck_weights[0] <= weight){
...
/*
다른 사람 풀이
if(onBridge.reduce((a, c) =>  a + c, 0) + truck_weights[0] <= weight)에서 false이면
else 문에서 현재 올라와있는 트럭이 나가는 시간으로 점프
*/