...
5 x 5 크기의 맵에, 당신의 캐릭터가 (행: 1, 열: 1) 위치에 있고, 상대 팀 진영은 (행: 5, 열: 5) 위치에 있는 경우의 예시입니다.
...
function solution(maps) {
    ...
    let directions = [[1, 0], [0, 1], [-1, 0], [0, -1]]; // 상, 하, 좌, 우 이동

    while (queue.length > 0) {
        let [r, c, dist] = queue.shift();

        if (r === height - 1 && c === width - 1) return dist; // 목표 도달

        for (let [dr, dc] of directions) {
            let nr = r + dr, nc = c + dc;
            if (nr >= 0 && nr < height && nc >= 0 && nc < width && maps[nr][nc] === 1) {
                maps[nr][nc] = 0; // 방문 처리
...
function solution(maps) {
    ...
    if((maps[height-2][width-1] == 0) && (maps[height-1][width-2] == 0)) return -1; -> height 또는 width가 1일 때 오류 발생

    function dfs(h, w, cnt, array){
        array[h][w] = 0;

        if((h == height - 1) && (w == width - 1)) return cnt;

        for(let i = h-1; i <= h+1; i++){
            for(let j = w-1; j <= w+1; j++){
                if(i >= 0 && i < height && j >= 0 && j < width){
                    if(array[i][j] == 1){
                        return dfs(i, j, cnt + 1, [...array]);
                    }
                }
            }
        }
